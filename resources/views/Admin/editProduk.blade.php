    ...
    <form action="/admin/produks/{{ $produk->id }}" class="m-auto w-50" method="POST" enctype="multipart/form-data">
      ...
      <div class="mb-5">
        ...
        <select class="form-select @error('pengguna') is-invalid @enderror" name="pengguna" id="pengguna"
          ...
          <option value="pria" @if($produk->detailproduk->pengguna == 'pria') selected @endif >Pria</option>
          <option value="wanita" @if($produk->detailproduk->pengguna == 'wanita') selected @endif >Wanita</option>
          <option value="anak" @if($produk->detailproduk->pengguna == 'anak') selected @endif>Anak</option>
        </select>
        @error('pengguna')
        <div class="invalid-feedback">{{ $message }}</div>
        @enderror
      ...
      <div class="mb-5 row">
        ...
        <div class="form-check col-2">
          <input class="form-check-input" type="checkbox" value="{{ $ukuran->id }}" id="ukuran{{ $i }}"
            name="ukuran{{ $i }}" @for($a=0; $a < $produk_ukuran->count(); $a++)
          @if((old('ukuran'.$i) ? : $produk_ukuran[$a])==$ukuran->id)
