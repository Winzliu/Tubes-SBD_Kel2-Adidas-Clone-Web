...
<div class="container p-4">
  ...
  <p class="mb-0">
    ...
    /
    <a class="text-dark fw-bold fs-6">Tambah Produk</a>
  </p>
  {{-- akhir path --}}
  ...
  <form action="/admin/produks" class="m-auto w-50" method="POST" enctype="multipart/form-data">
    ...
    <div class="mb-5">
      ...
      <textarea class="form-control @error('subDeskripsi') is-invalid @enderror" name="subDeskripsi" id="subDeskripsi"
        value="{{ old('subDeskripsi') }}">@if(old('subDeskripsi')) {{ old('subDeskripsi') }} @else You can't go wrong with a classic. These adidas.... @endif</textarea>
      @error('subDeskripsi')
    ...
    <div class="mb-5">
      ...
      <select class="form-select @error('pengguna') is-invalid @enderror" name="pengguna" id="pengguna"
        ...
        <option disabled @if (!old('pengguna')) selected @endif>Pengguna...</option>
        <option value="pria" @if (old('pengguna')=='pria' ) selected @endif>Pria</option>
        <option value="wanita" @if (old('pengguna')=='wanita' ) selected @endif>Wanita</option>
        <option value="anak" @if (old('pengguna')=='anak' ) selected @endif>Anak</option>
      </select>
      @error('pengguna')
      <div class="invalid-feedback">{{ $message }}</div>
      @enderror
